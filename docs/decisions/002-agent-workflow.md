...
| 역할 | 위치 | 책임 | 산출물 |
|---|---|---|---|
| Root Codex | 저장소 루트 | 프로젝트 총괄, PRD 작성, 제품 방향성, 게임 기획, 작업 초점, 제품 요구와 제약 정리 | `docs/product/prd.md`, `docs/current-focus.md`, 결정 문서 |
| Project Codex | `legacy-log-project/` | Root 기획 기준을 구현 가능한 기술 설계와 작업 지시서로 변환, 리뷰 반영, 최종 작업 지시서 작성 | 구현 계획, 최종 작업 지시서 |
| Project Claude | `legacy-log-project/` | 구현 계획 검토, 개선점 제안, 최종 지시서 기반 구현 | 리뷰 결과, 코드 변경, 검증 결과 |

## Root Codex 산출물 기준

Root Codex가 하위 프로젝트로 넘기는 문서는 PRD 기반의 제품 요구와 기획 기준을 포함한다. 다음 항목을 포함한다.
...
Root Codex는 게임 규칙, 콘텐츠 기준, 사건 목록, 작위/상태/태그/후계자 기획, 문체, 이름, UI/UX 검증 기준 같은 기획 요소를 사용자와 논의해 확정한다.
...
- Root 문서에 확정되지 않은 기획 세부가 구현 중 필요해졌을 때
...
이 흐름은 PRD와 구현을 바로 연결하지 않고, 계획과 리뷰 단계를 분리한다. 따라서 루트 Codex는 제품 방향성, 기획 기준, 요구 수준의 제약을 유지하고, Project Codex는 아키텍처와 구현 가능성을 구조화하며, Project Claude는 실제 코드 변경 전에 계획의 누락과 위험을 검토할 수 있다.
...
Root Codex와 Project Codex의 역할 경계는 `docs/decisions/016-root-planning-project-implementation-boundary.md`를 따른다.

Project Codex는 세부 기획을 임의로 확정하지 않는다. Project Codex는 Root가 확정한 기획을 구현 가능한 기술 설계, 데이터 구조, 작업 지시서로 변환한다.
