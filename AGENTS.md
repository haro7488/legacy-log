...
LegacyLog는 Godot + C#으로 만드는 가문 연대기 로그라이크 게임이다. 루트는 제품 방향, PRD, 게임 기획, 작업 초점, 하위 구현 위임을 관리하고, `legacy-log-project/`는 실제 Godot 구현을 담당한다.

게임의 기획 설계, 규칙, 콘텐츠 기준은 루트 문서에서 정한다. 기술적 데이터 저장 형식, 코드 구조, Godot 씬/리소스 구성은 하위 프로젝트의 구현 흐름에서 정한다.
...
이 항목들은 Project Codex와 Project Claude의 하위 구현 흐름에 위임한다. 게임 규칙, 사건, 작위, 상태, 유산 태그, 후계자, 이름, 문체, UI/UX 검증 기준 같은 기획 요소는 Root Codex가 사용자와 함께 정하고 루트 문서에 남긴다.
...
| 문서 | 대상 | 관점 | 다루는 것 |
|---|---|---|---|
| 루트 `AGENTS.md` | Root Codex | 프로젝트 총괄·PRD·기획 담당 | 제품 방향성, PRD, 게임 기획, 현재 초점, 제품 요구와 제약 |
| 루트 `CLAUDE.md` | Root Claude | 별도 루트 역할 | 이 문서에서 정의하지 않는다 |
| `legacy-log-project/AGENTS.md` | Project Codex | 구현 계획자·작업 지시자 | 기술 설계, 데이터 구조, 구현 계획, 리뷰 반영, 최종 작업 지시서 |
| `legacy-log-project/CLAUDE.md` | Project Claude | 구현 검토자·실행자 | 구현 계획 리뷰, 개선점 제안, 최종 지시서 기반 구현 |

**문서들은 내용을 중복시키지 않는다.** 루트 문서는 제품 방향성, PRD, 게임 기획, 요구 수준의 제약을 관리하고, 하위 문서는 기술 설계, 데이터 구조, 구현 계획, 실행 규칙을 관리한다.
...
### 원칙 4: 기획 공백과 구현 공백을 구분한다

게임 규칙, 콘텐츠, 상태, 작위, 사건, 문체, 이름, UI/UX 검증 기준 같은 기획 공백은 Root Codex가 사용자와 논의해 확정한다.

기술적 데이터 저장 형식, 코드 구조, Godot 씬/리소스 구성, 구현 설계 같은 구현 공백은 Project Codex/Project Claude 흐름에 넘긴다.

### 원칙 5: 기능 검증과 UI/UX 검증을 분리하지 않는다

구현 작업을 위임하거나 진행상황을 판단할 때는 기능 동작 여부와 UI/UX 수동 검증 여부를 함께 본다. 자동 검증이 성공해도 사용자가 직접 플레이하며 조작 흐름, 정보 이해 가능성, 텍스트 밀도, 버튼 흐름을 확인하지 못했다면 제품 경험 완료로 닫지 않는다.

하위 구현 지시에는 작업 의도, 사용자 조작 흐름, 기능 완료 기준, UI/UX 수동 검증 기준을 함께 포함한다.
...
- 트리거: 제품 방향, MVP 범위, 기능 요구사항, 게임 기획, 비범위가 정해졌거나 정리가 필요할 때.
- 작업: `docs/product/prd.md`에 사용자와 합의한 요구사항과 결정된 범위를 반영한다.
- 원칙: 기획 내용은 사용자와 합의해 루트 문서에 확정한다. 기술 구현 세부사항은 창작하지 않는다.
...
- 작업: Root Codex는 제품 요구, 기획 기준, 사용자 경험 범위, 제외 범위, 사전 가정, 제품 완료 기준, 보고 형식을 정리한다.
...
| Root 기획/Project 구현 경계 | [`016-root-planning-project-implementation-boundary.md`](docs/decisions/016-root-planning-project-implementation-boundary.md) |
| 기능 검증과 UI/UX 수동 검증 병행 | [`011-ui-ux-validation-workflow.md`](docs/decisions/011-ui-ux-validation-workflow.md) |
...
- **2026-05-19**: Root Codex는 기획 담당, Project Codex는 구현 담당으로 역할 경계를 명확화.
- **2026-05-19**: 기능 검증과 UI/UX 수동 검증을 함께 다루는 운영 원칙 추가.
